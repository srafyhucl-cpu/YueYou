...
void main() {
  ...
  for (final file in files) {
    ...
    content = content.replaceAllMapped(
      RegExp(r'provider\.isOver\s*=\s*(true|false);\s*provider\.score\s*=\s*(\d+);\s*provider\.board\s*=\s*List\.generate\([^;]+;'),
      (match) => 'provider.setStateForTesting(isOver: \${match[1]}, score: \${match[2]});'
    );
    
    content = content.replaceAll(
      RegExp(r'provider\.isOver\s*=\s*true;'),
