...
2. **yueyou-file-size-guard** - 单文件体量与上帝类反模式
   - 触发：修改 `.dart` 文件且行数 ≥ 警戒线、拆分 service/provider、新增公开类
   - 职责：防止单文件膨胀与上帝类复发
   - 检查：行数阈值、公开类 ≤ 3、公开方法 ≤ 25、禁用 `part`/`part of`
   - 联动工作流：`large-file-refactor-review`（必走）
...
```text
架构检查 → 文件体量 → 代码质量 → 测试验证 → 发布验收 → 任务收口
    ↓         ↓         ↓         ↓         ↓         ↓
domain   → file_size → constants → test   → release  → docs
```
...
- **文件体量冲突**：以 `yueyou-file-size-guard` 为准
...
开始开发
    ↓
是否修改的 .dart 文件行数 ≥ 警戒线？ → 是 → yueyou-file-size-guard + large-file-refactor-review
    ↓ 否
是否涉及架构变更？ → 是 → yueyou-architecture-guard
    ↓ 否
是否拆分/新增公开类？ → 是 → yueyou-file-size-guard
    ↓ 否
