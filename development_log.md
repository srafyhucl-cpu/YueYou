...
- **复核(release): 补齐 PR-7 文档验收勾选**：
  ...
  - `flutter analyze`、`dart analyze test`、AI 工程门禁（0 阻断、0 警告）、`go test ./...`、
    ...
    工具链仍交由 Ubuntu CI 验证。
  - `origin/main` 与 `origin/develop` 均已包含 `3c5351a`；发布说明同步校正 Tag 与分支提交关系。
...
- **复核(治理): 同步当前提交证据并保留未完成门禁**：
  - `origin/main`、`origin/develop`、`origin/yueyou_test` 均已包含 `3c5351a`；计划书、发布说明
    与日志中的旧分支哈希已校正。
...
- **测试(server): 补齐 TTS 取消与签名 TTL 回归**：
  - 新增等待队列取消、同键合成等待取消和签名 TTL 参数断言；上传失败、签名失败及合成取消
    路径与既有测试共同覆盖。
...
- **修复(ci): 恢复 Flutter 默认测试发现顺序**：
  - Run #245 测试步骤失败但没有具体用例 annotation；本地默认全量命令稳定通过，CI 的
    `find` 文件列表可能改变跨文件测试顺序并重新引入状态竞态。
  - 移除显式文件列表，恢复 Flutter 默认发现顺序，保留单进程覆盖率和诊断链路。
...
- **修复(ci): 隔离 Reader 集成测试进程**：
  - Run #250 返回 `base=1、tts=0、merge=0`，尾部停在 reader 集成测试，Tts 测试已独立通过。
...
- **重构(领域): PR-9 2048 纯领域内核完成**：
  ...
  - `GameProvider` 仅保留随机出生块、存档、音效、生命周期、ChangeNotifier 和 TTS 回调，
    从 547 行收敛至 322 行；随机数和所有外部副作用留在编排层。
  - 新增 `test/features/game_2048/game_engine_test.dart`，覆盖单次合并、连续合并、守恒、
    无合并移动和满盘边界。
...
- **评估(架构): PR-10 TTS/Reader 职责评估完成**：
  - `TtsEngineService` 当前 578 行，播放、下载/缓存、诊断和降级均已有独立可注入单元，
    继续拆分会增加 callback 与 Riverpod 生命周期耦合，决定不再拆分。
...
- **维护(调整外部验收门槛)**：根据当前无法招募五名真实参与者且不能等待两周 CI 的实际约束，
  将全局计划中的 G1 调整为“工程验证版”：由 Android 真机完成一次端到端验收，配合 5 组独立造数
  场景验证导入、朗读、恢复、重复句和跳句；明确造数不得冒充真实用户满意度或再次使用意愿。
- **维护(缩短 CI 稳定期)**：将连续两周 CI 稳定期调整为最近 3 次完整 push/PR CI 运行连续成功，
  且至少来自 2 个不同提交；真实用户可用性结论保留为未验证，不作为已完成证据。
- **文档**：同步更新 `DevelopmentPlan/20260711_全局治理与演进计划.md`，M4 仍须在工程验证门通过后
  以受限、可撤回方式启动。
