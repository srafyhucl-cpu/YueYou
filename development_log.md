...
- **维护(ai): AI 工程门禁收敛与检查器升级**：
  ...
  - **验证**：`dart scripts/ai_code_checker.dart` 通过（阻断 0 / warning 0）；`cmd /c "echo n| flutter analyze"` 通过（`No issues found!`）；`flutter test test/features/audio/tts_contract_test.dart test/features/audio/tts_audio_notifier_test.dart --concurrency=1` 通过。
