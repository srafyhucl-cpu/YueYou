...
- **修正(提示系统漏洞与状态同步)**:
  ...
  - 从 `TeleprompterView` 剥离了直接渲染错误代码的职责，统一交给全局 Toast 处理，维持提词器界面的视觉一致性。
  - 实现状态机自愈：当音频下载或降级播放成功一次后，TTS 引擎会自动调用 `_clearLastError` 解除报错状态。
  - `BoardMascot` 新增错误状态联动，当检测到 TTS 错误时，吉祥物的眼球和核心会切换为警告粉色并处于难过/闭眼表情。
- **修正(监听器变量丢失)**: 修复了重构过程中意外删除 `currentTimestamp` 导致 `fallbackNotification` 监听块崩溃的问题。
