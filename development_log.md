...
- **验收(PROD-00-P 浏览器运行时证据)**：
  - 使用 D 盘 Node 22 与 Playwright CLI 接入本地 HTML 原型，完成页面加载、关键点击路径、
    搜索框焦点、Tab 前进、滚轮行为、390x844/1440x1000 视口截图和本地素材请求检查。
  - 运行时截图写入 `docs/product/screenshots/20260715_浏览器验收_手机.png` 与
    `docs/product/screenshots/20260715_浏览器验收_桌面.png`，尺寸分别为 `390x844` 和
    `1440x1000`；请求清单仅包含本地 HTML 与本地 Xiaoyo 图片，均返回 200。
  - **验证**：Playwright 控制台错误/警告为 0；固定视口滚轮后滚动位置保持 0；原型合同仍为
    `ok=true`，Python 单测 `5 passed / 0 failed`。Android/iOS 真机、G0 签署、真实支付、
    退款、D0/D7、IP-0 六类外部证据仍未完成。
...
- **测试(PROD-00-P-2 引用截图尺寸合同)**：
  - 扩展 `scripts/check_product_prototype.py`，读取 PNG IHDR 头校验桌面 `1440x1000`、手机
    `390x844`，缺失、非 PNG 或尺寸不一致均阻断；不读取或改写图片内容。
  - 新增缺失截图回归测试，保持原型本地静态边界；**验证**：Python 单测 `5 passed / 0 failed`，
    原型检查 `ok=true`；`flutter analyze`、全量 Flutter 测试（753 passed / 4 skipped / 0 failed）、
    AI 门禁和 Go vet/build 通过；真实浏览器像素和交互仍未覆盖。

- **验收(PROD-00-P-3 引用截图人工视觉复核)**：
  - 人工查看桌面与手机截图，确认场景列、手机主体、审批观察点、底部播放器和三根导航均可见，
    未发现明显空白、裁切或互相遮挡。
  - 该结果只覆盖静态文件构图，不替代浏览器运行时、像素 diff、焦点/滚动、真机或支付证据。
...
- **功能(PROD-09-A 作者听校本地领域基础)**：
  - 新增纯 Dart `AuthorReviewMark` 与不可变 `AuthorReviewSession`，覆盖问题类型、章节
    字符区间、重复 ID、防止非法区间、按游标定位和复核状态。
  - 本地 JSON 导出只保留定位、备注和状态，不保存正文摘录、文件路径、账号或网络数据；
    未接入 UI、Provider、服务端、支付或普通读者主流程。
  - **验证**：作者听校定向测试 `5 passed / 0 failed`；Dart 分析无问题；作者样本、
    人工交付和真实购买/退款证据仍未提供，因此不宣称 PROD-09 商业实验成立。

- **功能(PROD-09-B 作者听校本地仓储)**：
  - 新增 `AuthorReviewRepository`、可注入 `AuthorReviewDocumentStore` 和应用文档目录
    实现；使用主文件、`.bak` 和 `.tmp` 完成原子写入，主文件损坏时回退备份，双份损坏
    时返回空会话。
  - 文件键使用 Base64 编码，避免书籍 ID 形成路径注入；仓储不上传正文，不接入账号、
    服务端、支付或普通阅读主线。
  - **验证**：作者 domain/仓储定向测试 `10 passed / 0 failed`；Dart 分析无问题；
    损坏文件测试只产生预期 `reader` 降级 warning，未出现未处理异常。

- **工具(PROD-07-B 匿名商业证据计算器)**：
  - 新增纯 Dart 计算器和 CLI，按详设第 21.2 节计算 Local Pro、书境、D7、Xiaoyo、作者
    听校和云音色指标，并同时报告总支付、退款和净支付。
  - 支付渠道、退款演练和负责人批准未全部通过时只输出 `notStarted` 或
    `insufficientEvidence`；输入含正文、书名、路径、账号、设备或用户字段时拒绝，未创建
    `commerce`、订单、权益或支付实现。
...
- **功能(PROD-05-B-1 TTS/Reader 语义事件桥)**：
  - 新增 `XiaoyoSignalBridge`，按游标和注入时间差生成稳定听读心跳，重复游标不重复
    发事件，单次补记限制为 1 至 90 秒；章节切换和 95% 完本阈值生成幂等事件。
  - 新增 Riverpod 桥接 Provider，价值系统开关关闭时不注册 TTS/Reader 监听；开启后
    只读取公开状态，Profile 仍由 `XiaoyoProfileNotifier` 和本地 Repository 持久化。
...
- **功能(PROD-06 21 天书境共读季本地累计活动)**：
  - 新增静态活动定义和四级里程碑；听读心跳自动累计活动秒数，支持受限的离线补记，
    跨过里程碑返回奖励事件，600 分钟解锁永久活动荣誉。
  - 活动不要求签到、不冲榜、不接远程配置，不增加 bondXp，不改变书境印记和付费权益；
    陪伴摘要展示活动分钟数、印记和荣誉，仍使用本地 Profile 主备份恢复。
